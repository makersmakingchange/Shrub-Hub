...
#define NEXT      PIN_SLEEVE
#define GROUND    PIN_RING2
...
int mode = 0; //zero through 3
#define MODE_CHANGE_DELAY        4000
//
void changeMode(){
  mode++;
  if(mode>3){
...
uint8_t Read_Buttons() {
  ...
  if (Current_Buttons = READ_SWITCHES) {
    ...
    do {
      ...
      if(Length_of_Press > MODE_CHANGE_DELAY) {
        Sample=0;   //If we've been here long enough, get out
        Current_Buttons = PUSHED_NONE;//trying to switch mode, not press a button
        changeMode();
        Serial.print("Changing Mode: Mode =");
        Serial.println(mode);
        delay(1000);
...
void Initialize_Buttons(void) {
  ...
  digitalWrite (GROUND,LOW);  //used as ground
};
